...
int executeCmd(HWND hWnd, HDC hdc, Socket socket, wstring tag, wstring content, Attribute attrs)
{
	...
	switch (cmd)
	{
		...
		case img:
		{
			...
			wcscat(newUrl, converter.from_bytes(socket.ipAddrs.c_str()).c_str());
			//string imgFile = converter.to_bytes(attrs.img.src);
			string file = converter.to_bytes(attrs.img.src);
			wcscat(newUrl, converter.from_bytes(file.c_str()).c_str());
