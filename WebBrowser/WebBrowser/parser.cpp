...
wstring Parser::getHyperRef(wstring content)
{
	wstring href = L"";
	content.erase(remove(content.begin(), content.end(), '\t'), content.end());
	wstring contentInfo = content;

	const wregex re(L"(<a[^>]*href=['|\"](.*?)['|\"].*?>)", regex::icase);

	wsmatch results;
	if (regex_match(contentInfo.cbegin(), contentInfo.cend(), results, re))
	{
		href = results[2];
		wcout << "Name = " << href << endl;
	}
	return href;
}
...
bool Parser::isEnglishLetter(wstring content)
{
	wstring letters = content;
	transform(letters.begin(), letters.end(), letters.begin(), ::tolower);
	wstring tester = L"abcdefghijklmnopqrstuvwxyz";

	for (int i = 0; i < letters.length(); i++)
	{
		int found = tester.find(letters[i]);
		if (found >= 0) 
			return true;
	}
	return false;
